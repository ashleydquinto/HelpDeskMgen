<?php
    ...
    */
    ...
    if($role == 'Users') { 
        if($action == 'show'){
            $sel = $conn -> query('SELECT * FROM request_table WHERE (requestor !="" AND rid = "'.$rid.'") ORDER BY date_created DESC');//WHERE requestor !="" ORDER BY date_created DESC 
        ...
        if($action == 'new'){
            $sel = $conn -> query(' SELECT * FROM request_table WHERE (requestor !="" AND status = "New" AND rid = "'.$rid.'") ORDER BY date_created DESC');//AND status ="available"
        ...
        if($action == 'ongoing'){
            $sel = $conn -> query(' SELECT * FROM request_table WHERE (requestor !="" AND status = "Ongoing" AND rid = "'.$rid.'") ORDER BY date_created DESC');//AND status ="available"
        ...
        if($action == 'pending'){
            $sel = $conn -> query(' SELECT * FROM request_table WHERE (requestor !="" AND status = "Pending" AND rid = "'.$rid.'") ORDER BY date_created DESC');//AND status ="available"
        ...
        if($action == 'resolved'){
            $sel = $conn -> query(' SELECT * FROM request_table WHERE (requestor !="" AND status = "Resolved" AND rid = "'.$rid.'") ORDER BY date_resolved DESC');//AND status ="available"
        ...
        if($action == 'closed'){
            $sel = $conn -> query(' SELECT * FROM request_table WHERE (requestor !="" AND status = "Closed" AND rid = "'.$rid.'") ORDER BY date_created DESC');//AND status ="available"
        ...
        if($action == 'cancelled'){
            $sel = $conn -> query(' SELECT * FROM request_table WHERE (requestor !="" AND status = "Cancelled" AND rid = "'.$rid.'") ORDER BY date_created DESC');//AND status ="available"
